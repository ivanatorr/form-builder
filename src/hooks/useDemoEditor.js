//this is demo hook from build-ui framework
//i installed it to have better experience
//of using this framework
//this hook is not in npm packages
...
const useDemoEditor = (params) => {
  ...
  function handlePositionedDrop(event, position) {
    ...
    const positionLeft =
      (dropPosition.left - dragPosition.left) / conversionLeft;
  ...
  const handleResize = (_event, bag) => {
    ...
    const { width, height, widthProp, heightProp } = sizeOnResizeStart.current;
